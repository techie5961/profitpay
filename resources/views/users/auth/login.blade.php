...
@section('main')
     <section class="column p-10  w-full flex-auto justify-center">
        {{-- <form action="{{ url('users/post/login/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.call_back)" class="w-full border-1 border-color-silver max-w-500 align-center br-10 bg-white  column g-5"> --}}
          <form style="backdrop-filter: blur(20px);box-shadow:inset 0 0 50px var(--bg-light)" action="{{ url('users/post/login/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.call_back)" class="w-full border-1 border-color-silver max-w-500 align-center br-10 bg-transparent c-white column g-5">
            ...
            <span style="font-family:titan one" class="c-primary desc">Welcome Back</span>
