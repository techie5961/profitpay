...
@section('main')
       <section class="column p-10  w-full flex-auto justify-center">
        {{-- <form action="{{ url('users/post/login/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.call_back)" class="w-full border-1 border-color-silver max-w-500 align-center br-10 bg-white  column g-5"> --}}
          <form style="backdrop-filter: blur(20px);box-shadow:inset 0 0 50px var(--bg-light)" action="{{ url('users/post/coupon/checker/process') }}" method="POST" onsubmit="document.querySelector('.coupon-summary').classList.add('display-none');PostRequest(event,this,MyFunc.call_back);" class="w-full border-1 border-color-silver max-w-500 align-center br-10 bg-transparent c-white column g-5">
            <div class="column w-full p-10 align-center g-5">
                <span class="desc c-primary" style="font-family:titan one;font-size:2rem;">Coupon Checker</span>
